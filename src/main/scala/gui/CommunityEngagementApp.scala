...
object CommunityEngagementApp extends JFXApp3 {
  ...
  private def createDashboardTab(): Tab = {
    ...
    val adminPanel = if (service.isCurrentUserAdmin) {
      new VBox {
        spacing = 10
        padding = Insets(20)
          ...
          new HBox {
            ...
            children = Seq(
              new Button("View All Users") {
                onAction = (_: ActionEvent) => showUsersDialog()
              },
              new Button("System Stats") {
                onAction = (_: ActionEvent) => showSystemStatsDialog()
              },
              new Button("Moderate Content") {
                onAction = (_: ActionEvent) => showModerationDialog()
              }
            )
          }
        )
      ...
      new VBox() // Empty for non-admins
    }
    
    val quickActionsPanel = new VBox {
      spacing = 10
      padding = Insets(20)
